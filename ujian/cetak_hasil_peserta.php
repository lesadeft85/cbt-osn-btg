...
// FIX #2: Validasi otorisasi akses via GET peserta_id
//         Peserta hanya boleh mencetak laporan MILIKNYA sendiri.
//         Akses via GET peserta_id kini diverifikasi terhadap session
//         atau token HMAC bertanda waktu (untuk window.open tanpa session).
// ============================================================
...
// ── Helper: buat & verifikasi token cetak ────────────────────
// Token = HMAC-SHA256(peserta_id|ujian_id|ts, APP_SECRET) dengan TTL 30 menit.
// Dipakai saat window.open dari selesai.php agar peserta lain tidak bisa
// mengakses laporan orang lain hanya dengan mengganti ?peserta_id=.
function buatTokenCetak(int $pid, int $ujianId): string {
    $secret = defined('APP_SECRET') ? APP_SECRET : 'changeme_secret_tka';
    $ts     = time();
    $payload = "$pid|$ujianId|$ts";
    $sig    = hash_hmac('sha256', $payload, $secret);
    return base64_encode("$payload|$sig");
}
function verifikasiTokenCetak(string $token, int $pid, int $ujianId): bool {
    $secret  = defined('APP_SECRET') ? APP_SECRET : 'changeme_secret_tka';
    $decoded = base64_decode($token, true);
    if ($decoded === false) return false;
    $parts = explode('|', $decoded);
    if (count($parts) !== 4) return false;
    [$tPid, $tUjian, $ts, $sig] = $parts;
    if ((int)$tPid !== $pid || (int)$tUjian !== $ujianId) return false;
    if (time() - (int)$ts > 1800) return false; // TTL 30 menit
    $payload  = "$tPid|$tUjian|$ts";
...
$tokenCetak = trim($_GET['token_cetak'] ?? '');
...
if ($kode) {
    // ── Akses via kode peserta (dari cek_nilai) ───────────────
    // Kode peserta adalah publik (tercetak di kartu ujian), tidak perlu auth tambahan.
    ...
    // ── Akses via GET peserta_id ──────────────────────────────
    // FIX #2: Validasi otorisasi berlapis:
    //   a) Jika ada session aktif → peserta_id harus cocok dengan session
    //   b) Jika tidak ada session → wajib ada token_cetak HMAC yang valid
    $sessionPid = (int)($_SESSION['peserta_id'] ?? 0);

    if ($sessionPid) {
        // Peserta masih dalam session — pastikan hanya bisa cetak miliknya
        if ($sessionPid !== $pidGet) {
            http_response_code(403);
            echo 'Akses ditolak: Anda tidak berhak mencetak laporan peserta lain.';
            exit;
        }
    } elseif ($tokenCetak) {
        // Session sudah habis tapi ada token HMAC (window.open dari selesai.php)
        if (!verifikasiTokenCetak($tokenCetak, $pidGet, $ujianId)) {
            http_response_code(403);
            echo 'Token cetak tidak valid atau sudah kadaluarsa.';
            exit;
        }
    } else {
        // Tidak ada session dan tidak ada token — tolak
        http_response_code(403);
        echo 'Akses tidak valid. Silakan login kembali.';
        exit;
    }
...
$lulus    = $nilai >= $kkm;
[$pred, $ket, $badge] = getNilaiPredikat((float)$nilai, (float)$kkm);
?>
<!DOCTYPE html>
<html lang="id">
<head>
