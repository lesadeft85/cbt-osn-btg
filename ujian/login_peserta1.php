        ...
        if ($tokenData !== null && empty($error)) {
            ...
            if (!$error) {
                ...
                if (!$pRow || $pRow->num_rows === 0) {
                    ...
                    // Fungsi pilihJadwalUntukPeserta() memilih jadwal spesifik kelas
                    // (misal Kelas 6) lebih diutamakan dari jadwal tanpa batasan kelas.
...
$kelasInfoTampil = '';
if ($jadwal && !empty($jadwal['kelas_diizinkan'])) {
    $kelasInfoTampil = $jadwal['kelas_diizinkan'];
}
?>
<!DOCTYPE html>
<html lang="id">
...
/* Kelas badge */
.kelas-info{background:#fefce8;border:1px solid #fde68a;border-radius:8px;padding:7px 12px;font-size:12px;font-weight:600;color:#854d0e;margin-bottom:14px;text-align:center}
...
<div class="wrap">
  ...
  <!-- Status Ujian — tampilkan semua jadwal aktif -->
  <?php if (!empty($jadwalAktifList)): ?>
  <?php foreach ($jadwalAktifList as $_jdl): ?>
  <div class="status-ujian aktif" style="margin-bottom:6px">
    <span class="live-dot"></span>
    Ujian berlangsung: <?= substr($_jdl['jam_mulai'],0,5) ?> – <?= substr($_jdl['jam_selesai'],0,5) ?> WIB
    <?php if (!empty($_jdl['nama_kategori'])): ?>
    &nbsp;·&nbsp; <strong><?= e($_jdl['nama_kategori']) ?></strong>
    <?php endif; ?>
    <?php if (!empty($_jdl['kelas_diizinkan'])): ?>
    &nbsp;·&nbsp;
    <?php foreach (array_filter(array_map('trim', explode(',', $_jdl['kelas_diizinkan']))) as $_kl): ?>
    <span style="background:rgba(255,255,255,.4);border-radius:6px;padding:1px 7px;font-size:11px;font-weight:800">Kelas <?= e($_kl) ?></span>
    <?php endforeach; ?>
    <?php endif; ?>
  </div>
  <?php endforeach; ?>
  ...
  <div style="margin-bottom:14px">
    ...
    <div id="sekolahPanel" style="display:none;background:#f8fafc;border:1.5px solid #e2e8f0;border-top:none;border-radius:0 0 8px 8px;padding:10px 12px;max-height:160px;overflow-y:auto">
      <?php foreach ($grouped as $jenjang => $sekolahs):
        // PHP 7.4 compat
        if (in_array($jenjang, ['SMP','MTS']))      $badgeColor = '#16a34a';
        elseif (in_array($jenjang, ['SMA','MA']))   $badgeColor = '#b45309';
        elseif ($jenjang === 'SMK')                 $badgeColor = '#dc2626';
        else                                         $badgeColor = '#1e3a8a';
      ?>
  ...
  <div class="form-section">
    ...
    <div class="error-alert">
      <span style="font-size:16px">⚠️</span><?= $error ?>
    </div>
    <?php endif; ?>
  ...
  <div class="footer-area">
    ...
    <div class="dev-text">
      ...
      <a href="<?= defined('LINK_TIKTOK') ? LINK_TIKTOK : 'https://www.tiktok.com/@mrkuncen' ?>" target="_blank">
