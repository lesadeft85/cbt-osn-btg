...
if ($conn->affected_rows === 0) {
    // Submit sudah diproses — ambil data hasil dari DB untuk isi session
    // FIX: tanpa ini, halaman selesai.php tampil kosong pada submit ganda
    $existRes = $conn->query(
        "SELECT jml_benar, jml_salah, jml_kosong, nilai, total_soal
         FROM hasil_ujian WHERE ujian_id=$ujianId AND peserta_id=$pesertaId LIMIT 1"
    );
    if ($existRes && $existRes->num_rows > 0) {
        $ex = $existRes->fetch_assoc();
        $_SESSION['hasil_benar']  = (int)$ex['jml_benar'];
        $_SESSION['hasil_salah']  = (int)$ex['jml_salah'];
        $_SESSION['hasil_kosong'] = (int)$ex['jml_kosong'];
        $_SESSION['hasil_nilai']  = (float)$ex['nilai'];
        $_SESSION['hasil_total']  = (int)$ex['total_soal'];
        $_SESSION['hasil_detail'] = [];
    }
    redirect(BASE_URL . '/ujian/selesai.php');
}
...
// BUG FIX #10: Hitung jumlah soal essay dari soal_order (bukan dari tabel jawaban)
// agar $totalSoalPg tidak deflated ketika peserta skip soal essay.
// Query JOIN di bawah hanya mengembalikan baris yang ADA di tabel jawaban,
// sehingga essay yang tidak dijawab tidak terhitung → $jmlEssay kurang → pembagi salah.
$jmlEssayDariOrder = 0;
if (!empty($soalOrder) && !empty($ids)) {
    $cntEssayRes = $conn->query(
        "SELECT COUNT(*) AS c FROM soal WHERE id IN ($ids) AND tipe_soal = 'essay'"
    );
    if ($cntEssayRes) {
        $jmlEssayDariOrder = (int)($cntEssayRes->fetch_assoc()['c'] ?? 0);
    }
}
...
$dijawab    = 0;
$adaEssay   = false;
$jmlEssay   = 0; // essay yang ada jawabannya (untuk deteksi adaEssay)
$kategoriId = null;
if ($statRes) {
    while ($row = $statRes->fetch_assoc()) {
        ...
        if ($row['tipe_soal'] === 'essay') {
            $adaEssay = true;
            $jmlEssay++;
            // Essay belum dinilai saat submit — tidak dihitung ke benar otomatis
        } elseif ($row['tipe_soal'] === 'mcma') {
...
$salah   = $dijawab - $benar;
$kosong  = max(0, $totalSoal - $dijawab);

// BUG FIX: soal essay TIDAK ikut denominator nilai PG saat submit.
// Sebelumnya ($benar / $totalSoal) menyebabkan nilai deflated karena
// soal essay dihitung sebagai "salah". Nilai akhir gabungan PG+Essay
// dihitung nanti di recalcEssayNilai() setelah admin menilai.
// BUG FIX #10: Gunakan $jmlEssayDariOrder (dari soal_order) bukan $jmlEssay (dari jawaban)
// agar denominator nilai PG tidak salah ketika ada soal essay yang tidak dijawab.
$jmlEssayFinal = max($jmlEssay, $jmlEssayDariOrder); // ambil yang lebih besar (aman)
$totalSoalPg = max(1, $totalSoal - $jmlEssayFinal);
$nilai = $adaEssay
    ? round(($benar / $totalSoalPg) * 100, 2)
...
$conn->query(
    "INSERT IGNORE INTO hasil_ujian
        (ujian_id, peserta_id, jadwal_id, kategori_id, total_soal, jml_benar, jml_salah, jml_kosong,
         nilai, ada_essay, waktu_mulai, waktu_selesai, durasi_detik)
     VALUES
        ($ujianId, $pesertaId, $jadwalId, $katStr, $totalSoal, $benar, $salah, $kosong,
         $nilai, " . ($adaEssay ? 1 : 0) . ", '{$ujian['waktu_mulai']}', NOW(), $duStr)"
);
...
$_SESSION['hasil_salah']  = $salah;   // FIX: simpan ke session agar selesai.php akurat
$_SESSION['hasil_kosong'] = $kosong;  // FIX: simpan ke session agar selesai.php akurat
