...
switch ($sortBy) {
    case 'pct_benar':
        $orderBy = 'pct_benar ASC'; break;
    case 'jml_jawab':
        $orderBy = 'jml_jawab DESC'; break;
    default:
        $orderBy = 'pct_salah DESC'; break;
}
...
usort($soalList, function($a, $b) use ($sortBy) {
    return ((($sortBy) === 'pct_benar') ? $a['pct_benar'] <=> $b['pct_benar'] : ((($sortBy) === 'jml_jawab') ? $b['jml_jawab'] <=> $a['jml_jawab'] : $b['pct_salah'] <=> $a['pct_salah']));
});
    ...
    <div class="card-body py-3">
        <form method="GET" class="row g-2 align-items-end">
            ...
            <div class="col-sm-2">
                ...
                <select name="tipe_soal" class="form-select form-select-sm" onchange="this.form.submit()">
    ...
    <div class="table-responsive">
        <table class="table table-hover align-middle mb-0 table-sm">
            ...
            <?php if ($soalList): $no=1; foreach ($soalList as $s):
                ...
                switch ($s['tipe_soal']) {
                    case 'bs':
                        $tipeLabel = '<span class="badge bg-warning text-dark">BS</span>'; break;
                    case 'mcma':
                        $tipeLabel = '<span class="badge" style="background:#7c3aed;color:#fff">MCMA</span>'; break;
                    default:
                        $tipeLabel = '<span class="badge bg-primary">PG</span>'; break;
                }
