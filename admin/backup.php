...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['restore'])) {
    csrfVerify();
    $filename = basename($_POST['restore']);
    $filepath = $backupDir . $filename;
    ...
    if ($sqlContent === false) {
        setFlash('error', 'Gagal membaca file backup.');
        redirect(BASE_URL . '/admin/backup.php');
    }
    ...
    foreach ($statements as $stmt) {
        ...
        if (!$conn->query($stmt)) {
            $errors_restore[] = $conn->error . ' — ' . substr($stmt, 0, 80);
        }
    }
    $conn->query("SET FOREIGN_KEY_CHECKS=1");

    if (empty($errors_restore)) {
        logActivity($conn, 'Restore database', $filename);
        setFlash('success', "✅ Database berhasil di-restore dari <strong>$filename</strong>.");
    } else {
        $errMsg = 'Restore selesai dengan ' . count($errors_restore) . ' error: <br>' . implode('<br>', array_slice($errors_restore, 0, 5));
    ...
    <div class="card-body p-0">
        ...
        <table class="table table-hover mb-0">
            ...
            <tr>
                ...
                <td>
                    <?php if ((strpos($bf['filename'], 'auto') === 0)): ?>
                ...
                <td class="text-center">
                    ...
                    <form method="POST" class="d-inline"
                          onsubmit="return confirm('⚠️ RESTORE dari file ini?\n\nSemua data saat ini akan DITIMPA dengan data dari backup:\n<?= $bf['filename'] ?>\n\nPastikan Anda sudah backup data terbaru sebelum restore!')">
<?= csrfField() ?>
                        <input type="hidden" name="restore" value="<?= htmlspecialchars($bf['filename']) ?>">
                        <button type="submit" class="btn btn-sm btn-outline-warning me-1" title="Restore dari backup ini">
