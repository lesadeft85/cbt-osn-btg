...
if (isset($_POST['action'])) {
    ...
    if ($action === 'clear_logs') {
        ...
        // Hapus SEMUA data ujian, jawaban, dan hasil ujian
        $conn->query("SET FOREIGN_KEY_CHECKS = 0");
        $q1 = $conn->query("TRUNCATE TABLE jawaban");
        $q2 = $conn->query("TRUNCATE TABLE ujian");
        $q3 = $conn->query("TRUNCATE TABLE hasil_ujian"); // BUG FIX: tabel ini tidak ikut dihapus sebelumnya
        $q4 = $conn->query("TRUNCATE TABLE token_ujian"); // hapus juga token yang sudah terpakai
        $conn->query("SET FOREIGN_KEY_CHECKS = 1");

        if ($q1 && $q2 && $q3) {
            logActivity($conn, 'Reset Total Ujian', 'Semua data jawaban, ujian, hasil_ujian, dan token dihapus');
            $msg = 'SEMUA data hasil ujian, jawaban, dan nilai peserta telah dihapus (Reset Total).';
        } else {
