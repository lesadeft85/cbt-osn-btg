...
function tipeSoalLabel(string $tipe): string {
    return ((($tipe) === 'bs') ? '<span class="badge bg-warning text-dark">BS</span>' : ((($tipe) === 'mcma') ? '<span class="badge bg-purple" style="background:#7c3aed">MCMA</span>' : ((($tipe) === 'essay') ? '<span class="badge bg-success">Esai</span>' : '<span class="badge bg-primary">PG</span>')));
}
...
function normalizeJawaban(string $tipe, $raw): string {
    ...
    if ($tipe === 'essay') {
        // Kunci jawaban esai = teks rubrik/contoh jawaban (bebas)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['aksi']??'') === 'tambah') {
    ...
    $tipe   = in_array($_POST['tipe_soal']??'pg', ['pg','bs','mcma','essay']) ? $_POST['tipe_soal'] : 'pg';
    ...
    if ($tipe === 'bs') {
        ...
        $pA = ''; $pB = ''; $pC = ''; $pD = '';
        // Kunci = rubrik/contoh jawaban (opsional)
        $jwb = trim($_POST['kunci_essay'] ?? '');
    } else {
        $pA = trim($_POST['pilihan_a'] ?? '');
        $pB = trim($_POST['pilihan_b'] ?? '');
        $pC = trim($_POST['pilihan_c'] ?? '');
    ...
    if (!$errors) {
        ...
        if (!$errors) {
            ...
            $essayBobot = ($tipe === 'essay') ? max(1, min(100, (int)($_POST['essay_bobot'] ?? 10))) : 10;
            // Auto-migrate: tambah kolom essay_bobot jika belum ada
            $_cbEb = $conn->query("SHOW COLUMNS FROM soal LIKE 'essay_bobot'");
            if (!$_cbEb || $_cbEb->num_rows === 0) {
                $alterOk = $conn->query("ALTER TABLE soal ADD COLUMN essay_bobot TINYINT UNSIGNED NOT NULL DEFAULT 10 AFTER pembahasan");
                if (!$alterOk) {
                    error_log("[SOAL][TAMBAH] Gagal ALTER TABLE essay_bobot: " . $conn->error);
                    $errors[] = 'Gagal migrasi kolom essay_bobot: ' . $conn->error;
                }
            }
            // ── DEBUG LOG: nilai yang akan di-INSERT ──
            error_log("[SOAL][TAMBAH] tipe=$tipe | kat=$katId | pert=" . mb_substr($pert,0,50)
                . " | jwb=" . var_export($jwb, true)
                . " | essayBobot=$essayBobot"
                . " | kunci_essay_post=" . var_export($_POST['kunci_essay'] ?? null, true));
            ...
            $st = $conn->prepare(
                "INSERT INTO soal (kategori_id, tipe_soal, pertanyaan, teks_bacaan, gambar,
                 ...
                 jawaban_benar, pembahasan, essay_bobot)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
            );
            if (!$st) {
                error_log("[SOAL][TAMBAH] prepare() GAGAL: " . $conn->error);
                $errors[] = 'DB prepare error: ' . $conn->error;
            } else {
                $st->bind_param('issssssssssssssi', $katId, $tipe, $pert, $teks_bacaan, $gambar,
                    $pA, $pB, $pC, $pD, $gpA, $gpB, $gpC, $gpD, $jwb, $pembahasan, $essayBobot);
                if (!$st->execute()) {
                    error_log("[SOAL][TAMBAH] execute() GAGAL: " . $st->error);
                    $errors[] = 'DB execute error: ' . $st->error;
                }
            }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['aksi']??'') === 'edit') {
    ...
    // Simpan state filter untuk restore setelah edit
    $_filterKatEdit  = trim($_POST['_filter_kat']  ?? '');
    $_filterTipeEdit = trim($_POST['_filter_tipe'] ?? '');
    $_filterQEdit    = trim($_POST['_filter_q']    ?? '');
    $teks_bacaan = trim($_POST['teks_bacaan'] ?? '');
    ...
    if (!$errors) {
        ...
        if (!$errors) {
            ...
            $essayBobotEdit = ($tipe === 'essay') ? max(1, min(100, (int)($_POST['essay_bobot'] ?? 10))) : 10;
            // ── DEBUG LOG: nilai yang akan di-UPDATE ──
            error_log("[SOAL][EDIT] id=$id | tipe=$tipe | kat=$katId | pert=" . mb_substr($pert,0,50)
                . " | jwb=" . var_export($jwb, true)
                . " | essayBobot=$essayBobotEdit"
                . " | kunci_essay_post=" . var_export($_POST['kunci_essay'] ?? null, true));
            ...
            $st = $conn->prepare(
                "UPDATE soal SET kategori_id=?, tipe_soal=?, pertanyaan=?, teks_bacaan=?, gambar=?,
                 pilihan_a=?, pilihan_b=?, pilihan_c=?, pilihan_d=?,
                 gambar_pilihan_a=?, gambar_pilihan_b=?, gambar_pilihan_c=?, gambar_pilihan_d=?,
                 jawaban_benar=?, pembahasan=?, essay_bobot=? WHERE id=?"
            );
            if (!$st) {
                error_log("[SOAL][EDIT] prepare() GAGAL: " . $conn->error);
                $errors[] = 'DB prepare error: ' . $conn->error;
            } else {
                $st->bind_param('issssssssssssssii',
                    $katId, $tipe, $pert, $teks_bacaan, $gambar,
                    $pA, $pB, $pC, $pD,
                    $gpA, $gpB, $gpC, $gpD,
                    $jwb, $pembahasan_edit, $essayBobotEdit, $id);
                if (!$st->execute()) {
                    error_log("[SOAL][EDIT] execute() GAGAL: " . $st->error);
                    $errors[] = 'DB execute error: ' . $st->error;
                }
                $st->close();
            }
            if (!$errors) {
                ...
                // Kembalikan ke halaman dengan filter yang sama
                $_params = [];
                if ($_filterKatEdit  !== '') $_params[] = 'kat='  . urlencode($_filterKatEdit);
                if ($_filterTipeEdit !== '') $_params[] = 'tipe=' . urlencode($_filterTipeEdit);
                if ($_filterQEdit    !== '') $_params[] = 'q='    . urlencode($_filterQEdit);
                redirect(BASE_URL . '/admin/soal.php' . (!empty($_params) ? '?' . implode('&', $_params) : ''));
            }
        }
    }
}
...
$statTipe = ['pg'=>0,'bs'=>0,'mcma'=>0,'essay'=>0];
if ($adaTipeSoal) {
...
<?php if (!empty($errors) && in_array($_POST['aksi'] ?? '', ['tambah','edit'])): ?>
<div class="alert alert-danger">
    <strong><i class="bi bi-exclamation-triangle me-1"></i>Gagal menyimpan soal:</strong>
    <ul class="mb-0 mt-1">
    ...
    <hr class="my-2">
    <small class="text-muted">Detail teknis juga dicatat di PHP error log server.</small>
</div>
<?php endif; ?>
...
<div class="row g-3 mb-3">
    ...
    <div class="col-6 col-md-3">
        <div class="card text-center border-0 shadow-sm">
            <div class="card-body py-2">
                <div class="fw-bold fs-4 text-success"><?= $statTipe['essay'] ?></div>
                <div class="small text-muted">Esai (Uraian)</div>
            </div>
        </div>
    </div>
</div>
...
<div class="card mb-3"><div class="card-body py-2">
    <form class="d-flex flex-wrap gap-2" method="GET">
        <select name="kat" class="form-select form-select-sm" style="width:200px" onchange="this.form.submit()">
        ...
        <select name="tipe" class="form-select form-select-sm" style="width:160px" onchange="this.form.submit()">
            ...
            <option value="essay" <?=$filterTipe==='essay'?'selected':''?>>Esai – Uraian</option>
        </select>
        ...
        <table id="tblSoal" class="table table-hover datatable mb-0">
            ...
            <tr>
                ...
                <td class="text-center">
                    ...
                    <?php
                    $_editParams = ['edit='.$s['id']];
                    if ($filterKat)  $_editParams[] = 'kat='.$filterKat;
                    if ($filterTipe) $_editParams[] = 'tipe='.urlencode($filterTipe);
                    if ($q)          $_editParams[] = 'q='.urlencode($q);
                    ?>
                    <a href="?<?=implode('&', $_editParams)?>" class="btn btn-sm btn-outline-warning btn-icon"><i class="bi bi-pencil"></i></a>
...
<?= csrfField() ?>
            ...
            <div class="modal-body" style="overflow-y:auto;max-height:calc(100vh - 200px)">
                ...
                <div class="row g-3 mb-3 pb-3 border-bottom">
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="tipe_soal" id="tipeTambah" class="form-select" onchange="ubahTipe(this,'tambah')">
                            ...
                            <option value="essay" <?=(($_POST['tipe_soal']??'')==='essay')?'selected':''?>>Esai – Uraian Bebas</option>
                        </select>
                    </div>
                    <div class="col-md-3">
                ...
                <div class="mb-3">
                    ...
                    <textarea name="teks_bacaan" class="form-control" rows="3" dir="auto"
                ...
                <div class="mb-3">
                    ...
                    <textarea name="pertanyaan" id="pertanyaanTambah" class="form-control" rows="3" required dir="auto"
                              placeholder="Tulis pertanyaan di sini... (gunakan $...$ untuk rumus, contoh: $x^2 + y^2 = r^2$)"
                              style="resize:vertical" oninput="updatePreview('pertanyaanTambah','prevPertanyaanTambah')"><?=htmlspecialchars($_POST['pertanyaan']??'')?></textarea>
                    <div class="form-text text-muted">
                        <i class="bi bi-info-circle me-1"></i>Rumus matematika: gunakan <code>$...$</code> untuk inline, <code>$$...$$</code> untuk baris penuh.
                        <a href="#" data-bs-toggle="collapse" data-bs-target="#panduanLatexTambah" class="ms-2 text-decoration-none" style="font-size:11px">📐 Lihat contoh →</a>
                    </div>
                    <div class="collapse" id="panduanLatexTambah">
                        <div class="card card-body py-2 px-3 mt-1" style="font-size:12px;background:#f8fafc">
                            <div class="row g-1">
                                <div class="col-6"><code>$x^2$</code> → pangkat</div>
                                <div class="col-6"><code>$\sqrt{x}$</code> → akar</div>
                                <div class="col-6"><code>$\frac{a}{b}$</code> → pecahan</div>
                                <div class="col-6"><code>$\pi, \alpha, \beta$</code> → simbol</div>
                                <div class="col-6"><code>$\leq, \geq, \neq$</code> → pertidaksamaan</div>
                                <div class="col-6"><code>$$\sum_{i=1}^n$$</code> → sigma</div>
                            </div>
                        </div>
                    </div>
                    <div id="prevPertanyaanTambah" class="mt-2 p-2 border rounded bg-light" style="display:none;font-size:14px;min-height:36px"></div>
                </div>
                    ...
                    <div class="row g-2">
                        ...
                        <div class="col-md-6">
                            <div class="input-group mb-1">
                                ...
                                <input type="text" name="pilihan_<?=$k?>" class="form-control" dir="auto"
                ...
                <div class="mb-3 p-3 bg-light rounded" id="blokJawabanTambah">
                    ...
                    <!-- Essay -->
                    <div id="jwbEssayTambah" style="display:none">
                        <div class="form-text mb-2">
                            <i class="bi bi-info-circle text-success"></i>
                            Soal esai dinilai secara manual oleh admin setelah ujian selesai.
                        </div>
                        <label class="form-label fw-semibold mb-1">Rubrik / Kunci Jawaban <small class="text-muted">(opsional)</small></label>
                        <textarea name="kunci_essay" class="form-control" rows="3" dir="auto"
                                  placeholder="Tulis rubrik atau contoh jawaban ideal…"
                                  style="font-size:13px"><?= htmlspecialchars($_POST['kunci_essay'] ?? '') ?></textarea>
                        <div class="mt-2">
                            <label class="form-label fw-semibold mb-1">Bobot Nilai <span class="text-danger">*</span></label>
                            <div class="input-group" style="max-width:200px">
                                <input type="number" name="essay_bobot" class="form-control"
                                       min="1" max="100" value="<?= (int)($_POST['essay_bobot'] ?? 10) ?>">
                                <span class="input-group-text">poin</span>
                            </div>
                            <div class="form-text">Nilai maks yang bisa diberikan untuk soal ini.</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-1">
                    ...
                    <textarea name="pembahasan" class="form-control" rows="2" dir="auto"
...
<?= csrfField() ?>
            ...
            <input type="hidden" name="_filter_kat"  value="<?=htmlspecialchars((string)$filterKat)?>">
            <input type="hidden" name="_filter_tipe" value="<?=htmlspecialchars($filterTipe)?>">
            <input type="hidden" name="_filter_q"    value="<?=htmlspecialchars($q)?>">
            <div class="modal-header bg-warning">
                <h5 class="modal-title"><i class="bi bi-pencil me-2"></i>Edit Soal</h5>
                <?php
                $_cancelParams = [];
                if ($filterKat)  $_cancelParams[] = 'kat='.$filterKat;
                if ($filterTipe) $_cancelParams[] = 'tipe='.urlencode($filterTipe);
                if ($q)          $_cancelParams[] = 'q='.urlencode($q);
                $_cancelUrl = BASE_URL . '/admin/soal.php' . (!empty($_cancelParams) ? '?' . implode('&', $_cancelParams) : '');
                ?>
                <a href="<?=$_cancelUrl?>" class="btn-close"></a>
            </div>
            <div class="modal-body">
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="tipe_soal" id="tipeEdit" class="form-select" onchange="ubahTipe(this,'edit')">
                            ...
                            <option value="essay" <?=$eTipe==='essay'?'selected':''?>>Esai – Uraian Bebas</option>
                        </select>
                    </div>
                    <div class="col-md-3">
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="teks_bacaan" class="form-control" rows="4" dir="auto"
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="pertanyaan" id="pertanyaanEdit" class="form-control" rows="3" required dir="auto"
                                  oninput="updatePreview('pertanyaanEdit','prevPertanyaanEdit')"><?=htmlspecialchars($editSoal['pertanyaan'])?></textarea>
                        <div class="form-text text-muted">
                            <i class="bi bi-info-circle me-1"></i>Gunakan <code>$...$</code> untuk rumus, contoh: <code>$2x + 5 = 15$</code>
                        </div>
                        <div id="prevPertanyaanEdit" class="mt-2 p-2 border rounded bg-light" style="display:none;font-size:14px;min-height:36px"></div>
                    </div>
                    <!-- Pilihan -->
                    <div id="blokPilihanEdit">
                        ...
                        <div class="col-md-6 mb-2" id="rowPilihan_<?=$k?>_e">
                            ...
                            <input type="text" name="pilihan_<?=$k?>" class="form-control mb-1" dir="auto"
                    ...
                    <div class="col-12">
                        ...
                        <div id="jwbEssayEdit" <?=$eTipe!=='essay'?'style="display:none"':''?>>
                            <div class="alert alert-success py-2 mb-2" style="font-size:13px">
                                <i class="bi bi-info-circle me-1"></i>
                                Soal esai dinilai manual oleh admin. Isi rubrik opsional.
                            </div>
                            <label class="form-label fw-semibold">Rubrik / Kunci Jawaban <small class="text-muted">(opsional)</small></label>
                            <textarea name="kunci_essay" class="form-control mb-2" rows="3" dir="auto"
                                      style="font-size:13px"><?= htmlspecialchars($eTipe==='essay' ? ($editSoal['jawaban_benar']??'') : '') ?></textarea>
                            <label class="form-label fw-semibold">Bobot Nilai</label>
                            <div class="input-group" style="max-width:200px">
                                <input type="number" name="essay_bobot" class="form-control"
                                       min="1" max="100" value="<?= (int)($editSoal['essay_bobot'] ?? 10) ?>">
                                <span class="input-group-text">poin</span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="row mt-3">
                    <div class="col-12">
                        ...
                        <textarea name="pembahasan" class="form-control" rows="3" dir="auto"
                                  placeholder="Jelaskan alasan jawaban yang benar..."
            ...
            <div class="modal-footer">
                <a href="<?=$_cancelUrl?>" class="btn btn-secondary">Batal</a>
...
function updatePreview(textareaId, previewId) {
    ...
    // Hanya tampilkan preview jika ada tanda $ (mengandung math)
    const hasMath = val.includes('$') || val.includes('\\(') || val.includes('\\[');
    if (!hasMath) { div.style.display = 'none'; return; }
    div.style.display = 'block';
    div.innerHTML = '<span class="text-muted" style="font-size:10px">Preview: </span>' + val.replace(/\n/g,'<br>');
...
// Jalankan preview untuk modal edit yang langsung terbuka — digabung ke DOMContentLoaded di bawah
...
function ubahTipe(sel, mode) {
    ...
    if (tipe === 'bs' || tipe === 'essay') {
        // BS & Essay: sembunyikan semua pilihan
    ...
    const jwbIds = ['jwbPg','jwbBs','jwbMcma','jwbEssay'];
    jwbIds.forEach(id => {
        const el = document.getElementById(id + suffix);
        if (el) el.style.display = 'none';
    });
    const map = {'pg':'jwbPg','bs':'jwbBs','mcma':'jwbMcma','essay':'jwbEssay'};
    const target = document.getElementById((map[tipe]||'jwbPg') + suffix);
    if (target) target.style.display = '';
}

// Jalankan saat load untuk modal edit yang langsung terbuka
document.addEventListener('DOMContentLoaded', () => {
    updatePreview('pertanyaanEdit',  'prevPertanyaanEdit');
    updatePreview('pertanyaanTambah','prevPertanyaanTambah');
