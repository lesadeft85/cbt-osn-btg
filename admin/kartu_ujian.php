...
$res = $conn->query("
    SELECT p.id, p.nama, p.kelas, p.kode_peserta, p.kode_sekolah,
...
$jr = $conn->query("
    SELECT j.id, j.tanggal, j.jam_mulai, j.jam_selesai, j.durasi_menit,
           j.keterangan, j.kategori_id, j.kelas_diizinkan, k.nama_kategori
    FROM jadwal_ujian j
...
// ── Helper: filter jadwal untuk kelas peserta ────────────────
// kelas_diizinkan kosong  = berlaku semua kelas
// kelas_diizinkan = "VI"  = kelas VI, VI A, VI B, dst
// kelas_diizinkan = "V,VI"= kelas V dan VI beserta sub-kelasnya
function jadwalUntukKelas(array $jadwal, string $kelasP): bool {
    $izin = trim($jadwal['kelas_diizinkan'] ?? '');
    if ($izin === '') return true;
    $kelasP = trim($kelasP);
    foreach (explode(',', $izin) as $k) {
        $k = trim($k);
        if ($kelasP === $k || strpos($kelasP, $k . ' ') === 0) return true;
    }
    return false;
}
...
// ── Kelas list dari DB (DISTINCT) — otomatis support VI A, VI B, dst ──
$kelasList = [];
$klRes = $conn->query("SELECT DISTINCT kelas FROM peserta WHERE kelas IS NOT NULL AND kelas != '' ORDER BY kelas");
if ($klRes) while ($r = $klRes->fetch_assoc()) $kelasList[] = $r['kelas'];
...
<link href="<?= defined('FONTS_NUNITO') ? FONTS_NUNITO : 'https://fonts.googleapis.com/css2?family=Nunito:wght@400;600;700;800;900&display=swap' ?>" rel="stylesheet">
<style>
/* ══════════════════════════════════════
   RESET & BASE
══════════════════════════════════════ */
...
<div class="toolbar">
  ...
  <form method="GET" style="display:contents">
    ...
    <select name="kelas" onchange="this.form.submit()">
      ...
      <option value="<?= htmlspecialchars($k) ?>" <?= $filterKelas===$k?'selected':''?>>Ruang <?= htmlspecialchars($k) ?></option>
      <?php endforeach; ?>
    </select>
...
foreach($peserta as $p):
  // Filter jadwal hanya yang berlaku untuk kelas peserta ini
  $jadwalPeserta = array_filter($jadwalList, fn($j) => jadwalUntukKelas($j, $p['kelas'] ?? ''));
  $jadwalPeserta = array_values($jadwalPeserta);
  $mapelSingle   = count($jadwalPeserta) === 1 ? ($jadwalPeserta[0]['nama_kategori'] ?? null) : null;

  $inisial2     = mb_strtoupper(mb_substr($p['nama'], 0, 2));
  $kelasStr     = $p['kelas'] ? ' ' . $p['kelas'] : '-';
  $kodeSekolah  = $p['kode_sekolah'] ?? '-';
  ...
  <div class="k-body">
    ...
    <div class="info-col">
      ...
      <div class="info-row">
        ...
        <span class="info-sep">:</span>
        <span class="info-val"><?= e($kodeSekolah) ?></span>
      </div>
      ...
      <div class="k-jadwal-list">
        ...
        <?php else: foreach($jadwalPeserta as $j): ?>
