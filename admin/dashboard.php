...
// Cek update dari GitHub — FIX: cache 24 jam di DB agar tidak blokir dashboard
require_once __DIR__ . '/../config/version.php';
$adaUpdate  = false;
$infoUpdate = [];

$_cacheKey  = 'update_check_cache';
$_cacheTtl  = 86400; // 24 jam
$_cacheData = getSetting($conn, $_cacheKey, '');
$_cacheValid = false;

if ($_cacheData) {
    $_cached = json_decode($_cacheData, true);
    if ($_cached && isset($_cached['ts']) && (time() - $_cached['ts']) < $_cacheTtl) {
        $infoUpdate  = $_cached['data'] ?? [];
        $_cacheValid = true;
    }
}

if (!$_cacheValid && function_exists('curl_init')) {
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, VERSION_URL);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_TIMEOUT, 3); // FIX: kurangi timeout 5→3 detik
    ...
    if ($jsonUpdate) {
        ...
        // Simpan ke cache DB
        setSetting($conn, $_cacheKey, json_encode(['ts' => time(), 'data' => $infoUpdate]));
    }
}

if ($infoUpdate && version_compare($infoUpdate['version'] ?? '0', APP_VERSION, '>')) {
    $adaUpdate = true;
}
unset($_cacheKey, $_cacheTtl, $_cacheData, $_cacheValid);
        ...
        <div class="row g-3 text-center">
            <div class="col-6 col-md-3">
                <div class="p-3 rounded-3" style="background:#eff6ff;border:1.5px solid #bfdbfe">
                    <div class="fw-900 text-primary" style="font-size:clamp(24px,6vw,36px)" id="rtOnline"><?= $sedangUjian ?></div>
            ...
            <div class="col-6 col-md-3">
                <div class="p-3 rounded-3" style="background:#f0fdf4;border:1.5px solid #bbf7d0">
                    <div class="fw-900 text-success" style="font-size:clamp(24px,6vw,36px)" id="rtSelesai"><?= $totalUjianSelesai ?></div>
            ...
            <div class="col-6 col-md-3">
                <div class="p-3 rounded-3" style="background:#fefce8;border:1.5px solid #fde68a">
                    <div class="fw-900 text-warning" style="font-size:clamp(24px,6vw,36px)" id="rtTotal"><?= $totalPeserta ?></div>
            ...
            <div class="col-6 col-md-3">
                <div class="p-3 rounded-3" style="background:#fdf4ff;border:1.5px solid #e9d5ff">
                    <div class="fw-900 text-purple" style="font-size:clamp(24px,6vw,36px);color:#7c3aed" id="rtNilai"><?= $nilaiRata ?></div>
...
<div class="row g-3 mb-3">
    <div class="col-12 col-lg-8">
        <div class="card h-100">
            ...
            <div class="card-body">
                <canvas id="chartSekolah" style="max-height:260px"></canvas>
                <?php if (!$g1Labels): ?>
    ...
    <div class="col-12 col-lg-4">
        <div class="card h-100">
            ...
            <div class="card-body d-flex flex-column align-items-center justify-content-center">
                <canvas id="chartDist" style="max-height:200px"></canvas>
            </div>
        </div>
    </div>
</div>
...
<div class="row g-3 mb-4">
    <div class="col-12 col-lg-6">
        <div class="card h-100">
            ...
            <div class="card-body">
                <canvas id="chartPeserta" style="max-height:220px"></canvas>
            </div>
        </div>
    </div>
    <div class="col-12 col-lg-6">
        ...
        <div class="card h-100">
            ...
            <div class="card-body p-3">
                ...
                <a href="<?= BASE_URL ?>/<?= $l['url'] ?>"
                   class="btn btn-outline-<?= $l['color'] ?> text-start d-flex align-items-center gap-2 py-2 w-100">
                    <i class="<?= $l['icon'] ?>"></i><span class="text-truncate"><?= $l['label'] ?></span>
                </a>
                </div>
                <?php endforeach; ?>
                </div>
        ...
        <div class="card h-100">
            ...
            <div class="card-body p-0">
                ...
                <table class="table table-hover mb-0">
                    <thead><tr>
                        <th>Nama Peserta</th>
                        <th class="d-none d-lg-table-cell">Kode Sekolah</th>
                        <th class="d-none d-sm-table-cell">Ruang</th>
                    ...
                    <tr>
                        <td>
                            ...
                            <span class="d-sm-none text-muted text-xs d-block"><?= e($r['kode_sekolah'] ?? '-') ?> · <?= e($r['nama_sekolah'] ?? '-') ?></span>
                            <span class="d-sm-none text-muted text-xs d-block"><?= $r['waktu_selesai'] ? date('d/m H:i', strtotime($r['waktu_selesai'])) : '-' ?></span>
                        </td>
                        <td class="text-sm d-none d-lg-table-cell"><code><?= e($r['kode_sekolah'] ?? '-') ?></code></td>
                        <td class="text-sm d-none d-sm-table-cell"><?= e($r['nama_sekolah'] ?? '-') ?></td>
...
new Chart(document.getElementById('chartSekolah'), {
    ...
    options: {
        ...
        scales: {
            ...
            x: { ticks: { font: { size: 10 }, maxRotation: 45, autoSkip: true, maxTicksLimit: 6 } }
        }
    }
});
            ...
            if (d.baru_selesai && d.baru_selesai.length > 0) {
                ...
                listBaru.innerHTML = d.baru_selesai.map(p =>
                    `<span class="badge bg-success px-2 py-1">
                        ${p.nama} <small class="opacity-75">(${p.kode_sekolah || '-'} · ${p.sekolah})</small>
