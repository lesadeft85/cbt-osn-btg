...
/* Teks Bacaan per-soal (tampil di dalam kartu) */
.teks-bacaan-box{background:#f0f9ff;border-left:4px solid #1a56db;border-radius:0 8px 8px 0;padding:14px 16px;margin-bottom:14px;font-size:13.5px;line-height:2;color:#1e293b}
.teks-bacaan-box p{text-indent:2em;margin-bottom:0.5em}
/* Wacana bersama (tampil sekali di atas kelompok soal) */
.wacana-group{background:#fff;border:2px solid #bfdbfe;border-radius:14px;margin-bottom:20px;box-shadow:0 2px 8px rgba(30,90,200,.07)}
.wacana-group-head{background:#eff6ff;padding:10px 20px;font-size:12px;font-weight:700;color:#1a56db;border-bottom:1px solid #bfdbfe;display:flex;align-items:center;gap:6px;border-radius:12px 12px 0 0}
.wacana-group-body{padding:18px 24px;font-size:14px;line-height:2.1;color:#1e293b}
.wacana-group-body p{text-indent:2em;margin-bottom:0.5em}
.wacana-group-body p:first-child{margin-top:0}
.soal-card-body.has-wacana{grid-template-columns:1fr}
.soal-card-body.has-wacana .soal-divider{display:none}
.soal-card-body.has-wacana .soal-kiri{padding:16px 22px 0}
.soal-card-body.has-wacana .soal-kanan{padding:0 22px 16px;border-top:none}
...
<?php
// Helper: render teks bacaan → paragraf proper (pisah per baris kosong)
function renderWacana(string $teks): string {
    $teks = htmlspecialchars($teks, ENT_QUOTES, 'UTF-8');
    // Pisah per baris kosong → paragraf; baris tunggal → br
    $blocks = preg_split('/\n{2,}/', $teks);
    $out = '';
    ...
    return $out ?: '<p>' . nl2br($teks) . '</p>';
}

$prevWacana = null; // track wacana sebelumnya untuk grouping
foreach ($soalList as $i => $s):
  $no = $i + 1;
  $tipe = $s['tipe_soal'] ?? 'pg';
  $jwbBenar = $s['jawaban_benar'] ?? '';
  $wacana   = trim($s['teks_bacaan'] ?? '');
  switch ($tipe) {
      case 'bs':
          $tipeBadge = '<span class="tipe-badge" style="background:#fef3c7;color:#92400e">BS</span>'; break;
      case 'mcma':
          $tipeBadge = '<span class="tipe-badge" style="background:#ede9fe;color:#5b21b6">MCMA</span>'; break;
      case 'essay':
          $tipeBadge = '<span class="tipe-badge" style="background:#dcfce7;color:#15803d">Esai</span>'; break;
      default:
          $tipeBadge = '<span class="tipe-badge" style="background:#eff6ff;color:#1d4ed8">PG</span>'; break;
  }
  $jwbBenarArr = $tipe === 'mcma' ? explode(',', $jwbBenar) : [$jwbBenar];

  // Tampilkan wacana sekali jika berbeda dari sebelumnya
  if ($wacana !== '' && $wacana !== $prevWacana):
      // Hitung berapa soal yang punya wacana sama (berikutnya)
      $jmlSoalWacana = 0;
      for ($j = $i; $j < count($soalList); $j++) {
          if (trim($soalList[$j]['teks_bacaan'] ?? '') === $wacana) $jmlSoalWacana++;
          else break;
      }
?>
<div class="wacana-group">
  <div class="wacana-group-head">
    <i class="bi bi-book-fill"></i>
    Bacalah teks berikut dengan seksama untuk menjawab soal
    <?= $jmlSoalWacana > 1 ? "nomor $no–" . ($no + $jmlSoalWacana - 1) : "nomor $no" ?>!
  </div>
  <div class="wacana-group-body">
    <?= renderWacana($wacana) ?>
  </div>
</div>
<?php
...
<div class="soal-card">
  ...
  <div class="soal-card-body <?= $adaWacana ? 'has-wacana' : '' ?>">
    <!-- Kiri: gambar + pertanyaan (wacana sudah tampil di atas sebagai group) -->
    <div class="soal-kiri">
