...
// BUG FIX: APP_VERSION dibutuhkan oleh footer.php — load version.php jika belum
if (!defined('APP_VERSION') && file_exists(__DIR__ . '/../config/version.php')) {
    require_once __DIR__ . '/../config/version.php';
}
if (!defined('APP_VERSION')) {
    define('APP_VERSION', '1.0.0');
}
// Load centralized external resource paths
if (file_exists(__DIR__ . '/../config/paths.php')) {
    require_once __DIR__ . '/../config/paths.php';
}
$user = getCurrentUser();

// ── Auto-restore tentang.php jika terhapus ──────────────────
(function() {
    $target  = __DIR__ . '/../admin/tentang.php';
    $backup1 = __DIR__ . '/../../.sys/tentang.php';
    $backup2 = __DIR__ . '/../.sys/tentang.php';
    if (!file_exists($target)) {
        if (file_exists($backup1)) {
            @copy($backup1, $target);
        } elseif (file_exists($backup2)) {
            @copy($backup2, $target);
        }
    }
})();
...
<head>
    ...
    <!-- DataTables -->
    <link href="<?= defined('CDN_DATATABLES_CSS') ? CDN_DATATABLES_CSS : 'https://cdn.datatables.net/1.13.7/css/dataTables.bootstrap5.min.css' ?>" rel="stylesheet">
    <!-- KaTeX (render rumus matematika) -->
    <link rel="stylesheet" href="<?= defined('CDN_KATEX_CSS') ? CDN_KATEX_CSS : 'https://cdn.jsdelivr.net/npm/katex@0.16.10/dist/katex.min.css' ?>">
    <!-- Custom CSS -->
    ...
    <script>
        ...
        <style>
            /* Truncate long app name in sidebar to avoid overflowing/wrapping */
            .sidebar .brand-name{
                display: -webkit-box;
            ...
            .sidebar .brand-sub{font-size:12px;color:rgba(255,255,255,0.7)}
...
<aside class="sidebar" id="sidebar">
    ...
    <div class="sidebar-brand">
        ...
        <div class="brand-text">
            <div class="brand-name" title="<?= htmlspecialchars($namaAplikasi) ?>"><?= htmlspecialchars($namaAplikasi) ?></div>
    ...
    <ul class="sidebar-menu mt-3">
        ...
        <li>
            ...
            <ul class="sidebar-submenu collapse <?= $isMaster?'show':'' ?>" id="menuMaster">
                <li><a href="<?= BASE_URL ?>/admin/sekolah.php" class="<?= ($activeMenu??'')==='sekolah'?'active':'' ?>">Kelola Tes</a></li>
        ...
        <!-- Laporan & Hasil -->
        <?php $isLaporan = in_array($activeMenu, ['nilai', 'hasil', 'rekap_sekolah', 'rekap_kelas', 'export', 'exportpdf', 'analisis_soal', 'kartuujian', 'sertifikat', 'nilai_essay']); ?>
        <li>
            ...
            <ul class="sidebar-submenu collapse <?= $isLaporan?'show':'' ?>" id="menuLaporan">
                ...
                <li><a href="<?= BASE_URL ?>/admin/nilai_essay.php" class="<?= ($activeMenu??'')==='nilai_essay'?'active':'' ?>">
                    <i class="bi bi-pencil-square me-1"></i>Penilaian Esai</a></li>
        ...
        <!-- Pengaturan Sistem -->
        <?php $isSistem = in_array($activeMenu, ['display', 'backup', 'log', 'pengaturan', 'about', 'maintenance', 'tentang']); ?>
        <li>
            ...
            <ul class="sidebar-submenu collapse <?= $isSistem?'show':'' ?>" id="menuSistem">
                ...
                <li><a href="<?= BASE_URL ?>/admin/tentang.php" class="<?= in_array($activeMenu??'', ['about','tentang'])?'active':'' ?>">Tentang Aplikasi</a></li>
            </ul>
        </li>
    ...
    <ul class="sidebar-menu mt-3">
        ...
        <!-- Pelaksanaan Ujian -->
        <?php $isUjian = in_array($activeMenu, ['mulai_tes', 'hasil', 'kartu_ujian', 'monitoring', 'cetak_daftar_hadir']); ?>
        <li>
            ...
            <ul class="sidebar-submenu collapse <?= $isUjian?'show':'' ?>" id="menuUjianSekolah">
                ...
                <li><a href="<?= BASE_URL ?>/sekolah/monitoring.php" class="<?= ($activeMenu??'')==='monitoring'?'active':'' ?>"><i class="bi bi-activity me-1 text-danger"></i>Monitoring Live</a></li>
                ...
                <li><a href="<?= BASE_URL ?>/sekolah/cetak_daftar_hadir.php" class="<?= ($activeMenu??'')==='cetak_daftar_hadir'?'active':'' ?>">Daftar Hadir</a></li>
            </ul>
        </li>

        <!-- Laporan -->
        <?php $isLaporan = in_array($activeMenu, ['rekap_nilai', 'ranking']); ?>
        <li>
            <a href="#menuLaporanSekolah" class="has-dropdown <?= $isLaporan?'active':'' ?>" data-bs-toggle="collapse" aria-expanded="<?= $isLaporan?'true':'false' ?>">
                <span class="menu-icon"><i class="bi bi-bar-chart-fill"></i></span> Laporan & Rekap
            </a>
            <ul class="sidebar-submenu collapse <?= $isLaporan?'show':'' ?>" id="menuLaporanSekolah">
                <li><a href="<?= BASE_URL ?>/sekolah/rekap_nilai.php" class="<?= ($activeMenu??'')==='rekap_nilai'?'active':'' ?>">Rekap Nilai</a></li>
                <li><a href="<?= BASE_URL ?>/sekolah/ranking.php" class="<?= ($activeMenu??'')==='ranking'?'active':'' ?>">Ranking Peserta</a></li>
            </ul>
        </li>

        <!-- Pengaturan -->
        <?php $isSetting = in_array($activeMenu, ['profil', 'tentang']); ?>
        <li>
            ...
            <ul class="sidebar-submenu collapse <?= $isSetting?'show':'' ?>" id="menuSetting">
                ...
                <li><a href="<?= BASE_URL ?>/sekolah/tentang.php" class="<?= ($activeMenu??'')==='tentang'?'active':'' ?>">Tentang Aplikasi</a></li>
            </ul>
        </li>
    </ul>
    <?php endif; ?>
...
<div class="main-wrapper">
    ...
    <?php
    // ── Warning lisensi hampir expired ───────────────────────
    if (isset($_SESSION['lisensi_warning'])) {
        $hariSisa = (int)$_SESSION['lisensi_warning'];
        $warnClass = $hariSisa <= 3 ? 'danger' : 'warning';
        $icon      = $hariSisa <= 3 ? '🚨' : '⚠️';
        echo "<div class='alert alert-{$warnClass} alert-dismissible fade show py-2 mb-3' role='alert' style='font-size:13px'>"
           . "{$icon} <strong>Lisensi hampir kedaluwarsa!</strong> "
           . "Sisa <strong>{$hariSisa} hari</strong>. "
           . "<a href='" . BASE_URL . "/aktivasi.php' class='alert-link'>Perpanjang sekarang</a>."
