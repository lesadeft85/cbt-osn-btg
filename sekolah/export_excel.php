...
if (!class_exists('\ZipArchive')) {
    ...
    fputcsv($output, []);
    fputcsv($output, ['No', 'Nama Peserta', 'Kode', 'Kode Sekolah', 'Kelas', 'Mapel', 'Benar', 'Salah', 'Kosong', 'Total', 'Nilai', 'Predikat', 'Status', 'Durasi (mnt)', 'Tgl Selesai']);
...
$rekapData[] = [['value' => $namaSekolah . ($npsn ? ' | NPSN: '.$npsn : '') . ' | Kode Sekolah: ' . $kodeSekolah, 'style' => 0]];
...
foreach ($mapelGroups as $namaMapel => $mapelRows) {
    ...
    $mapelData[] = [['value' => $namaSekolah . ' | Kode Sekolah: ' . $kodeSekolah . ' | TP: ' . $tahunPelajaran, 'style' => 0]];
    $mapelData[] = [];
