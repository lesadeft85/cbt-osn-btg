...
// Pre-load jumlah soal per kategori (hindari N+1 query di dalam loop)
$soalCounts = [];
$_scRes = $conn->query("SELECT kategori_id, COUNT(*) AS c FROM soal GROUP BY kategori_id");
if ($_scRes) while ($_sc = $_scRes->fetch_assoc()) $soalCounts[(int)$_sc['kategori_id']] = (int)$_sc['c'];
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-sm table-hover mb-0">
                        <thead>
                            <tr><th>Nama</th><th>Kode Peserta</th><th>Kode Sekolah</th><th>Ruang</th><th class="text-center">Status</th></tr>
                        </thead>
                        <tbody>
                            <?php
                            ...
                            <tr>
                                ...
                                <td><code><?= htmlspecialchars($b['kode_sekolah'] ?? '-') ?></code></td>
                                <td><?= htmlspecialchars($b['kelas'] ?? '-') ?></td>
                                <td class="text-center"><span class="badge bg-warning text-dark">Belum Ujian</span></td>
                            </tr>
                            <?php endwhile; else: ?>
                            <tr><td colspan="5" class="text-center text-muted py-3">Semua peserta sudah mengikuti ujian 🎉</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
