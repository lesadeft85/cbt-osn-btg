...
if ($q)           $conds[] = "(p.nama LIKE '%" . $conn->real_escape_string($q) . "%'
                               OR p.kode_peserta LIKE '%" . $conn->real_escape_string($q) . "%')";
...
$hasilRes = $conn->query("
    SELECT h.nilai, h.waktu_mulai, h.waktu_selesai,
           ...
           p.id AS peserta_id, p.nama, p.kode_peserta, p.kode_sekolah, p.kelas,
...
$katRes  = $conn->query("
    ...
    LEFT JOIN jadwal_ujian jd ON jd.id = h.jadwal_id
    LEFT JOIN kategori_soal k ON k.id = COALESCE(h.kategori_id, jd.kategori_id)
    WHERE p.sekolah_id = $sekolahId AND k.id IS NOT NULL
...
@media print {
    body > *, .main-wrapper, .sidebar, .content-wrapper, .content-area, .topbar {
    ...
    .ps-box { text-align: center; border: 1px solid #e2e8f0; border-radius: 6px; padding: 6px 4px; }
    ...
    .pt-wrap th {
        ...
        padding: 6px 5px; text-align: left; border: 1px solid #1547b8;
    }
    .pt-wrap th.c, .pt-wrap td.c { text-align: center; }
    .pt-wrap td {
        padding: 5px; border: 1px solid #f1f5f9;
    ...
    .pf-wrap {
        margin-top: 16px; border-top: 1px solid #d1d5db; padding-top: 10px;
        display: flex; justify-content: space-between;
        align-items: flex-end; font-size: 9.5px; color: #6b7280;
    }
    .pf-ttd { text-align: center; }
    .pf-ttd-line { border-bottom: 1px solid #374151; width: 160px; margin: 48px auto 4px; }
    ...
    <div class="ph-wrap">
        ...
        <div class="ph-text">
            ...
            <small>
                ...
                <?php if ($filterKat && !empty($katList)): ?>
                    &nbsp;·&nbsp; Mapel:
                    <?php foreach ($katList as $kat): if ($kat['id'] == $filterKat) echo e($kat['nama_kategori']); endforeach; ?>
                <?php endif; ?>
                <?php if ($q): ?> &nbsp;·&nbsp; Cari: "<?= e($q) ?>"<?php endif; ?>
            </small>
        </div>
    </div>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th class="c" style="width:72px">Kode</th>
                    <th class="c" style="width:40px">Sekolah</th>
                    <th style="width:90px">Mapel</th>
                    <th class="c" style="width:38px">Benar</th>
                    <th class="c" style="width:36px">Salah</th>
                    <th class="c" style="width:40px">Kosong</th>
                    <th class="c" style="width:42px">Nilai</th>
                    <th class="c" style="width:90px">Predikat</th>
                    <th class="c" style="width:55px">Status</th>
                       <th class="c" style="width:72px">Kode Sekolah</th>
                       <th class="c" style="width:40px">Ruang</th>
                    <th class="c" style="width:44px">Durasi</th>
                    <th style="width:78px">Tgl Selesai</th>
                </tr>
            </thead>
            <tbody>
            <?php if ($rows):
            ...
            <tr class="<?= $rowClass ?>">
                <td class="c" style="font-weight:700">
                    <?= ((int)$r['rank']===1?'🥇':((int)$r['rank']===2?'🥈':((int)$r['rank']===3?'🥉':$r['rank']))) ?>
                </td>
                <td><?= e($r['nama']) ?></td>
                ...
                <td class="c"><strong style="font-size:13px;color:<?= $pcolor ?>"><?= $r['nilai'] ?></strong></td>
                <td class="c">
                ...
                <td class="c">
                    ...
                    <?php else: ?>
                        <span class="lulus-no">✗ Tidak</span>
                    <?php endif; ?>
                </td>
            ...
            <?php endforeach; else: ?>
            <tr><td colspan="13" style="text-align:center;padding:16px;color:#94a3b8">Belum ada data ujian</td></tr>
            <?php endif; ?>
            </tbody>
        </table>
    </div>
            ...
            <ul class="dropdown-menu dropdown-menu-end">
                <li>
                    <a class="dropdown-item"
                       href="<?= BASE_URL ?>/sekolah/cetak_hasil.php<?= $exportParams ? '?' . $exportParams : '' ?>"
                       target="_blank">
                        <i class="bi bi-list-ol me-2"></i>Cetak Ranking
                    </a>
                </li>
                <li>
                    <a class="dropdown-item"
                       href="<?= BASE_URL ?>/sekolah/cetak_hasil.php?mode=ledger<?= $exportParams ? '&' . $exportParams : '' ?>"
                       target="_blank">
                        <i class="bi bi-grid me-2"></i>Cetak Ledger
                    </a>
                </li>
                <li><hr class="dropdown-divider"></li>
                <li>
                    <button class="dropdown-item" onclick="window.print()">
                        <i class="bi bi-file-pdf me-2"></i>Cetak Halaman Ini
                    </button>
                </li>
            </ul>
        </div>
    ...
    <div class="card-header p-0 border-bottom-0">
        <ul class="nav nav-tabs" id="hasilTab" role="tablist">
            <li class="nav-item" role="presentation">
                <button class="nav-link active py-3 px-4 fw-bold" data-bs-toggle="tab"
                        data-bs-target="#ranking" type="button" role="tab">
                    <i class="bi bi-trophy-fill text-warning me-2"></i>Ranking Peserta
                </button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="nav-link py-3 px-4 fw-bold" data-bs-toggle="tab"
                        data-bs-target="#ledger" type="button" role="tab">
                ...
                <div class="table-responsive">
                    <table id="tblHasilSekolah" class="table table-hover datatable mb-0">
                        ...
                        <tr>
                            ...
                            <td><small class="badge bg-info text-dark"><?= e($r['nama_kategori']) ?></small></td>
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover mb-0 datatable">
                        ...
                        <tbody>
                        <?php if ($ledgerData): $no = 1;
                              foreach ($ledgerData as $pid => $p):
                                  $pNilai = $p['nilai'];
                        ...
                        <tr>
                            ...
                            <td class="text-center"><code style="font-size:11px"><?= e($p['kode_sekolah'] ?? '-') ?></code></td>
                            ...
                            <?php endforeach; ?>
                            <td class="text-center table-primary fw-bold fs-6"><?= $avg ?></td>
                        </tr>
                        <?php endforeach; else: ?>
                        <tr><td colspan="<?= 6 + count($mapelList) ?>" class="text-center text-muted py-5">
