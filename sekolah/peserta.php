...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['aksi']??'') === 'edit') {
    ...
    $kodeSek = trim($_POST['kode_sekolah'] ?? '');
    if ($nama) {
        $st = $conn->prepare("UPDATE peserta SET nama=?,kelas=?,kode_sekolah=? WHERE id=? AND sekolah_id=?");
        $st->bind_param('sssii', $nama, $kelas, $kodeSek, $id, $sekolahId);
        $st->execute(); $st->close();
...
if ($filterKelas) $cond .= " AND p.kelas='" . $conn->real_escape_string($filterKelas) . "'";

// Daftar kelas untuk dropdown filter
$_krRes  = $conn->query("SELECT DISTINCT kelas FROM peserta WHERE sekolah_id=$sekolahId AND kelas IS NOT NULL AND kelas!='' ORDER BY kelas");
$kelasList = [];
if ($_krRes) while ($kr = $_krRes->fetch_assoc()) $kelasList[] = $kr['kelas'];
...
<div class="page-header">
    ...
    <div class="d-flex gap-2">
        <a href="<?=BASE_URL?>/sekolah/export_peserta.php<?= ($q||$filterKelas) ? '?'.http_build_query(array_filter(['q'=>$q,'kelas'=>$filterKelas])) : '' ?>"
           class="btn btn-outline-success" title="Export daftar peserta ke Excel">
...
<div class="card mb-3"><div class="card-body py-2">
    <form class="d-flex gap-2 flex-wrap" method="GET" id="formFilter">
        ...
        <select name="kelas" class="form-select form-select-sm" style="width:150px"
            ...
            <option value="<?=htmlspecialchars($kls)?>" <?=$filterKelas===$kls?'selected':''?>>
                Ruang <?=htmlspecialchars($kls)?>
            </option>
            <?php endforeach; ?>
        </select>
        <?php endif; ?>
        <button class="btn btn-sm btn-outline-primary"><i class="bi bi-search me-1"></i>Cari</button>
        <?php if($q||$filterKelas): ?><a href="?" class="btn btn-sm btn-outline-secondary">Reset</a><?php endif; ?>
    </form>
</div></div>
...
<div class="card">
    ...
    <div class="card-body p-0"><div class="table-responsive">
        <table id="tblPesertaSekolah" class="table table-hover datatable mb-0">
            <thead><tr>
                <th>#</th><th>Nama</th><th>Kode Peserta</th><th>Ruang</th><th>Kode Sekolah</th>
        ...
        <form method="POST" class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="kode_sekolah" class="form-control" value="<?=htmlspecialchars($_POST['kode_sekolah']??'')?>" placeholder="(opsional)">
                </div>
    ...
    <div class="modal-dialog">
        <form method="POST" class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="mb-3">
                    <label class="form-label fw-semibold">Kode Sekolah</label>
                    <input type="text" name="kode_sekolah" class="form-control" value="<?=htmlspecialchars($editData['kode_sekolah']??'')?>" placeholder="(opsional)">
                </div>
            </div>
            <div class="modal-footer">
