...
/* ── PERBAIKAN: Redirect Mendukung URL Dinamis BASE_URL ──────── */
function redirect(string $url): void {
    // Jika URL adalah path lokal/relatif (tidak diawali http:// atau https://)
    if (strpos($url, 'http://') !== 0 && strpos($url, 'https://') !== 0) {
        // Gabungkan BASE_URL dengan path target, pastikan tidak ada double slash
...
/* ── PERBAIKAN: Type Casting pada Log Aktivitas ──────────────── */
...
function getKelasByJenjang(string $jenjang): array {
    ...
    switch (strtoupper($jenjang)) {
        case 'SD': case 'MI':               $base = ['I','II','III','IV','V','VI']; break;
        case 'SMP': case 'MTS':             $base = ['VII','VIII','IX']; break;
        case 'SMA': case 'MA': case 'SMK':  $base = ['X','XI','XII']; break;
        default: $base = ['I','II','III','IV','V','VI','VII','VIII','IX','X','XI','XII'];
    }
    $result = [];
    foreach ($base as $k) {
        foreach ($sub as $s) {
...
function renderKelasOptions(string $selected = '', string $jenjang = ''): string {
    if ($jenjang) {
        $kelasList = getKelasByJenjang($jenjang);
    } else {
        $kelasList = [];
        foreach (['SD', 'SMP', 'SMA'] as $j) {
            $kelasList = array_merge($kelasList, getKelasByJenjang($j));
        }
    }
...
/* ── PHP 7.4-compatible predikat nilai ─────────────────────── */
function getNilaiPredikat(float $nilai, float $kkm = 60.0): array {
    if ($nilai >= 90) return ['A', 'Istimewa',         'success', '#059669'];
    if ($nilai >= 80) return ['B', 'Sangat Baik',      'success', '#16a34a'];
    if ($nilai >= 70) return ['C', 'Baik',             'info',    '#0ea5e9'];
    if ($nilai >= $kkm) return ['D', 'Cukup',          'warning', '#f59e0b'];
    return              ['E', 'Perlu Bimbingan',        'danger',  '#ef4444'];
}

// ============================================================
// Cache Invalidation untuk jawaban peserta
// ============================================================

function invalidateJawabanCache(mysqli $db, int $ujianId): void {
    $db->query(
...
function isJawabanCacheValid(mysqli $db, int $ujianId): bool {
    $res = $db->query(
